...
use mipidsi::{models::ST7789, options::{ColorInversion, Rotation}, Builder};
...
async fn main(_spawner: Spawner) {
    ...
    display.fill_solid(&Rectangle::new(Point::new(0, 0), Size::new(DISPLAY_WIDTH as u32, DISPLAY_HEIGHT as u32)), Rgb565::RED).unwrap();
    Timer::after_millis(1000).await;

    display.fill_solid(&Rectangle::new(Point::new(0, 0), Size::new(DISPLAY_WIDTH as u32, DISPLAY_HEIGHT as u32)), Rgb565::GREEN).unwrap();
    Timer::after_millis(1000).await;

    display.fill_solid(&Rectangle::new(Point::new(0, 0), Size::new(DISPLAY_WIDTH as u32, DISPLAY_HEIGHT as u32)), Rgb565::BLUE).unwrap();
    Timer::after_millis(1000).await;

    // Test 2: Draw shapes
    display.fill_solid(&Rectangle::new(Point::new(0, 0), Size::new(DISPLAY_WIDTH as u32, DISPLAY_HEIGHT as u32)), Rgb565::BLACK).unwrap();
    ...
    Text::with_baseline("Display: 240x135", Point::new(10, 40), text_style, Baseline::Top)
        .draw(&mut display).unwrap();

    Text::with_baseline("All systems OK!", Point::new(10, 60), text_style, Baseline::Top)
        .draw(&mut display).unwrap();
    ...
    loop {
        for i in 0..30 {
            ...
            // Static text
            Text::with_baseline("Animation Test", Point::new(10, 10), text_style, Baseline::Top)
                .draw(&mut display).unwrap();
